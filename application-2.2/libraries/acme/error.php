...
class Error {
	...
	*/
    public function show_error($header = '', $message = '', $template = 'error_general', $status_code = 500, $log_error = true)
    {
		...
		// Carrega view
		echo $this->CI->template->load_view('_errors/' . $template, array('header' => $header, 'message' => $message), true, false);
		exit;
    }
	...
	*/
	public function show_php_error($severity = '', $message = '', $filepath = '', $line = 0, $log_error = true)
	{
		...
		echo $this->CI->template->load_view('_errors/error_php', array('severity' => $severity, 'message' => $message, 'filepath' => $filepath, 'line' => $line), true, false);
	}
	...
	*/
    ...
    {
		$this->CI =& get_instance();
		echo $this->CI->template->load_view('_errors/exception_page', array('message' => $message), true, false);
		exit;
    }
	...
	*/
	...
	{
		$this->CI =& get_instance();
		echo $this->CI->template->load_view('_errors/error_404', array(), true, false);
		exit;
	}
}