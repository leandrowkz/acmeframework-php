...
class App_Access extends ACME_Core_Controller {
	...
	*/
	public function login()
	{
		...
		// Load view
		$this->template->load_view('login', $args, false, false);
	}
	...
	*/
	...
	{
		if( ! $process)
			$this->template->load_view($this->view_dir . '/forgot_password', array(), false, false);
		else {
			...
			// user doesnt exist
			if( count($user) <= 0 )
				$this->template->load_view($this->view_dir . '/forgot_password', array('error' => lang('Ops! There is no users registered with this email address')), false, false);
				...
				$body_msg = $this->template->load_view('_acme/app_user/email_reset_password', $args, true, false);
			    ...
			    if( ! @$this->email->send() )
					$this->template->load_view($this->view_dir . '/forgot_password', array('error' => lang('Ops! It was not possible to send the email message')), false, false);
				else {
					...
					// load page success!
					$this->template->load_view($this->view_dir . '/forgot_password', array('success' => lang('OK! One message containing all steps to reset your password was forwarded to ') . $email), false, false);
	...
	*/
	public function reset_password($id_user = 0, $key_access = '', $process = false)
	{
		...
		if( ! $process)
			$this->template->load_view($this->view_dir . '/reset_password', array('id_user' => $id_user, 'key_access' => $key_access), false, false);
		else {
			...
			// load view (update ok)
			$this->template->load_view($this->view_dir . '/reset_password', array('id_user' => $id_user, 'key_access' => $key_access, 'success' => lang('OK! Password successfully changed')), false, false);
		}
	}
