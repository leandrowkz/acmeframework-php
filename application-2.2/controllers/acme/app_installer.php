...
class App_Installer extends ACME_Core_Controller {
	...
	*/
	public function system_requirements()
	{
		...
		// Loads view
		$this->template->load_view('_acme/app_installer/system_requirements', $args, false, false);
	}
	...
	*/
	public function new_app_info($process = false)
	{
		...
		// Just loads page
		if( ! $process)
			$this->template->load_view('_acme/app_installer/new_app_info', $args, false, false);
	...
	*/
	public function summary()
	{
		if( $this->session->userdata('installed') === true )
			$this->template->load_view('_acme/app_installer/summary', array(), false, false);
		else
			redirect('app_installer');
	}
