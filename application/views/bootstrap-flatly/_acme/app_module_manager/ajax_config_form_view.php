...
<script type="text/javascript" language="javascript">
	$(document).ready(function(){
		// Tabela de campos
		ajax_load_box_config_form_fields('view', <?php echo $id_module ?>);
	});
</script>
<div>
	...
	<br />
	<br />
	<h5><?php echo lang('Formulário de Visualização') ?></h5>
	<hr />
		...
		<div id="box_group_view">
			<div>
				...
				<div id="field_view" style="max-width:2000px;min-width:300px;">
					<div>
						<a href="javascript:void(0);" onclick="show_area('instrucoes_form_view')"><?php echo lang('Exibir Instruções') ?></a>
						<img src="<?php echo URL_IMG ?>/bullet_arrow_down.png" />
					</div>
					<div class="font_11" id="instrucoes_form_view" style="display:none;">					
						...
						<div><?php echo lang('&bull;&nbsp;Este formulário é exibido quando uma ação de registro da listagem do módulo apontar seu link e sua chave primária para') . ' <strong>' . $controller . '/form/view/{VALOR_CHAVE_PRIMARIA_DO_REGISTRO}</strong>.'?></div>
						<div><?php echo lang('&bull;&nbsp;A ação básica deste formulário é permitir a visualização de um registro da tabela do módulo selecionado (tabela: ') . ' <strong>' . $table_name . '</strong>).' ?></div>
			...
			<div class="odd">
				...
				<div id="field_view" class="font_11">
					<div><a href="javascript:void(0)" onclick="show_area('dados_modulo_form_view')"><?php echo lang('Exibir Detalhes')?></a></div>
					<div id="dados_modulo_form_view" style="display:none;">
			...
			<div>
				...
				<div id="field_view" class="top">
					<input type="hidden" id="lang_status_form_view_enable" value="<?php echo lang('Habilitado') ?>"/>
					<input type="hidden" id="lang_status_form_view_disable" value="<?php echo lang('Desabilitado') ?>"/>
					<div class="inline top" style="margin:4px 2px 0 0"><input type="checkbox" id="checkbox_form_view" onclick="ajax_set_config_form('view', <?php echo $id_module?>)" style="margin-top:2px;" <?php echo (get_value($form, 'dtt_inative') != '' || count($form) <= 0) ? '' : 'checked="checked"'; ?>  /></div>
					<?php if(get_value($form, 'dtt_inative') != '' || count($form) <= 0){?>
					<h6 id="status_form_view" class="inline top font_error"><?php echo lang('Desabilitado') ?></h6>
					<?php } else { ?>
					<h6 id="status_form_view" class="inline top font_success"><?php echo lang('Habilitado') ?></h6>
					<?php } ?>
					<div class="inline top font_11" style="margin:3px 0 0 5px;"><a href="javascript:void(0)" onclick="show_area('detalhes_form_view')"><?php echo lang('Exibir Detalhes')?></a></div>
					<div id="detalhes_form_view" style="display:none">
						<div class="font_11">
			...
			<div class="odd">
				...
				<div id="field_view" class="top">
					<input type="hidden" id="lang_status_action_view_enable" value="<?php echo lang('Sim') ?>" />
					<input type="hidden" id="lang_status_action_view_disable" value="<?php echo lang('Não') ?>" />
					<div class="inline top" style="margin:4px 2px 0 0"><input type="checkbox" id="checkbox_action_view" onclick="ajax_set_config_action('view', <?php echo $id_module?>)" style="margin-top:2px;" <?php echo (get_value($action, 'dtt_inative') != '' || count($action) <= 0) ? '' : 'checked="checked"'; ?>  /></div>
					<?php if(get_value($action, 'dtt_inative') != '' || count($action) <= 0){?>
					<h6 id="status_action_view" class="font_error inline top"><?php echo lang('Não') ?></h6>
					<?php } else { ?>
					<h6 id="status_action_view" class="font_success inline top"><?php echo lang('Sim') ?></h6>
					<?php } ?>
					<div class="inline top font_11" style="margin:3px 0 0 5px;"><a href="javascript:void(0)" onclick="show_area('detalhes_action_view')"><?php echo lang('Exibir Detalhes')?></a></div>
					<div id="detalhes_action_view" style="display:none">
						<div class="font_11">
							...
							<div><a href="javascript:void(0);" onclick="iframe_modal('<?php echo lang('Edição de dados da Ação')?>', '<?php echo URL_ROOT ?>/acme_module_manager/ajax_modal_update_action/view/<?php echo get_value($module, 'id_module') ?>', '<?php echo URL_IMG ?>/icon_update.png', 700, 480)"><?php echo lang('Editar dados da Ação')?></a></div>
						</div>
						</div>
					</div>
				</div>
	...
	<hr />
	<div id="box_table_form_fields_view"></div>
	<br />
