...
<body>
	<div id="modal_content">
		...
		<?php echo message('warning', lang('ATENÇÃO!'), lang('A permissão que está sendo visualizada será deletada. Isso implicará em uma correção nos arquivos que utilizam a validação desta permissão, para este módulo. Caso queira confirmar a deleção, clique em <strong>ok</strong>, caso contrário clique em <strong>cancelar</strong>.')) ?>
		
		<!-- FORMULARIO -->
		<form id="form_default" name="form_default" action="<?php echo URL_ROOT ?>/acme_module_manager/ajax_modal_module_permission_delete_process" method="post">
			...
			<br />
			<h5 class="font_shadow_gray inline top"><?php echo lang('Dados da Permissão') ?></h5>
			<hr style="margin-bottom:5px" />
			<div id="box_group_view">
				<div class="odd">
					<div id="label_view"><?php echo lang('ID (#)') ?></div>
					<div id="field_view"><?php echo get_value($data, 'id_module_permission') ?></div>
				</div>
				<div>
					<div id="label_view"><?php echo lang('Rótulo da Permissão') ?></div>
					<div id="field_view"><?php echo lang(get_value($data, 'lang_key_rotule')) ?></div>
				</div>
				<div class="odd">
					<div id="label_view"><?php echo lang('Nome da Permissão') ?></div>
					<div id="field_view"><?php echo get_value($data, 'permission') ?></div>
				</div>
				<div>
